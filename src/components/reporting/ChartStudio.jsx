/**
 * ChartStudio — the report-component workshop of the Reporting suite.
 *
 * Interactive chart exploration with drill-down, cross-filtering, and SVG
 * export — used to craft visual components for reports. Extracted from the
 * former Visualizations page; page shell/header now lives in Reporting.jsx.
 */
...
import {
  ...
  Network, Download, RefreshCw, X, Link2, SlidersHorizontal, FilePlus2
} from "lucide-react";
...
export default function ChartStudio({ onSendToReport }) {
  const { addLog } = useApp();
  ...
  if (loading) return (
    <div className="flex items-center justify-center py-24" style={{ color: "var(--text-muted)" }}>
      <RefreshCw size={20} className="animate-spin mr-2" /> Loading chart studio...
    </div>
  );
    ...
    <div className="flex flex-col">
      ...
      <div className="rounded-xl px-4 py-2.5 mb-3 flex items-center gap-3 flex-wrap"
        ...
        <button onClick={handleExportSVG}
          className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all shrink-0"
          title="Download the current chart as an SVG report component"
        ...
        {onSendToReport && (
          <button onClick={onSendToReport}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition-all shrink-0"
            title="Open the Report Builder to assemble this into a report"
            style={{ background: "rgba(254,221,0,0.1)", border: "1px solid rgba(254,221,0,0.35)", color: "var(--accent-primary)" }}>
            <FilePlus2 size={12} /> Build Report
          </button>
        )}
      </div>
        ...
        <div className="viz-zone flex flex-col">
          ...
          <div className="viz-widget-card">
            ...
            <ul className="space-y-1.5 text-xs relative z-10" style={{ color: "var(--text-secondary)" }}>
              ...
              <li className="flex gap-2">
                <span style={{ color: "#40c4ff" }}>·</span>
                <span>Export the current view as an SVG report component, or hit <span style={{ color: "var(--accent-primary)" }}>Build Report</span> to assemble it into a full report.</span>
              </li>
            </ul>
          </div>
        </div>

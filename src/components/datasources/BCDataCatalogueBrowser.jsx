...
export default function BCDataCatalogueBrowser({ onImport, onClose }) {
  ...
  const search = async (e) => {
    ...
    setExpanded(null);
    try {
      const res = await base44.functions.invoke("bcDataCatalogue", { action: "search", query: query.trim(), limit: 20 });
  ...
  const QUICK_SEARCHES = ["Indigenous health", "mental health", "chronic disease", "mortality", "substance use", "maternal"];
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: "rgba(0,0,0,0.65)" }}>
      <div className="flex flex-col rounded-xl shadow-2xl overflow-hidden"
        ...
        <div className="flex items-center justify-between px-5 py-3 border-b shrink-0"
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <div className="text-xs" style={{ color: "var(--text-muted)" }}>catalogue.data.gov.bc.ca · CKAN API</div>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-3 space-y-2">
          ...
          {!loading && results.length === 0 && !query && (
            <div className="text-center py-12 text-xs" style={{ color: "var(--text-muted)" }}>
              Search the BC Data Catalogue to find and import health-related datasets.
            </div>
          )}
            ...
            return (
              <div key={ds.id} className="rounded-lg overflow-hidden"
                ...
                <div className="flex items-start gap-3 px-4 py-3">
                  <button onClick={() => toggleExpand(ds)} className="mt-0.5 shrink-0"

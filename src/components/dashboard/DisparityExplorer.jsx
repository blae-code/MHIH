          ...
          <div className="flex gap-1">
            {CHART_TYPES.map(ct => (
              <button key={ct.id} onClick={() => { setChartType(ct.id); setDrillItem(null); }}
                ...
                style={{ background: chartType === ct.id ? "var(--accent-primary)" : "var(--bg-overlay)", color: chartType === ct.id ? "#000" : "var(--text-muted)", border: `1px solid ${chartType === ct.id ? "var(--accent-primary)" : "var(--border-subtle)"}` }}>
                <ct.icon size={11} />
                {ct.label}
              </button>
            ))}

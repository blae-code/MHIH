...
const CKAN_BASE = "https://catalogue.data.gov.bc.ca/api/3/action";
...
Deno.serve(async (req) => {
  try {
    ...
    if (action === "search") {
      ...
      if (!data.success) throw new Error(data.error?.message || "CKAN returned error");
      ...
      const datasets = (data.result?.results || []).map(pkg => ({
        ...
        url: `https://catalogue.data.gov.bc.ca/dataset/${pkg.name}`,
    ...
    if (action === "show") {
      ...
      const res = await fetch(url, { headers: { "Accept": "application/json" } });
      if (!res.ok) throw new Error(`CKAN show failed: ${res.status}`);
      const data = await res.json();
      if (!data.success) throw new Error(data.error?.message || "CKAN returned error");
